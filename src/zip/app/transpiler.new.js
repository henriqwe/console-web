...
const ycl_transpiler = {
        ...
        while (index < tokens.length) {
            if (from == 0 && tokens[index].symbol == 'schema') {
                ...
                if (!ycl_transpiler.ycl_reserved_word_contains(schema.name) 
                    ...
                    throw new Error('error: the token format is not lowercase, or matches some reserved word, or does not match only alpha characters. token \''+tokens[index + 1].symbol+'\', line '+tokens[index + 1].line+', position: '+tokens[index + 1].position+'. [from: '+from+']');
                }
                ...
                if (!ycl_transpiler.ycl_reserved_word_contains(actual_entity.name) 
                    ...
                    if (actual_entity.command == 'd') {
                        ...
                        if (index == tokens.length) {
                            throw new Error ('error: neither a next entity was found, nor the end of the code after the entity declared to be delete.');
                        }
                    } else {
                ...
                while (tokens[index].symbol  != ')') {
                    if (tokens[index].symbol == 'sql') {
                        ...
                        code_body = code_body + '    ' + tokens[index].symbol + '\n';
                        index++;
                    } else if (actual_entity._conf.dbType == 'sql' 
                        ...
                        while (tokens[index].symbol  != ']') {
                            if (!ycl_transpiler.ycl_reserved_word_contains(tokens[index].symbol) 
                                ...
                                throw new Error('error: attribute name incorrect into uniqueKey. token \''+tokens[index].symbol+'\', line '+tokens[index].line+', position: '+tokens[index].position+'. [from: '+from+']');
                            }
                        }
                        code_body = code_body + '    ' + tokens[index].symbol + '\n'; // o ']' do 'uniqueKey ['
                        index++;
                        ...
                        while (tokens[index].symbol  != ']') {
                            if (!ycl_transpiler.ycl_reserved_word_contains(tokens[index].symbol) 
                                ...
                                throw new Error('error: attribute name incorrect into indexKey. token \''+tokens[index].symbol+'\', line '+tokens[index].line+', position: '+tokens[index].position+'. [from: '+from+']');
                            }
                        }
                        code_body = code_body + '    ' + tokens[index].symbol + '\n'; // o ']' do 'indexKey ['
                        index++;
                            ...
                            if (tokens[index].symbol == 'read' && tokens[index + 1].symbol == '[') {
                                ...
                                while (tokens[index].symbol  != ']') {
                                    if (!ycl_transpiler.ycl_reserved_word_contains(tokens[index].symbol) 
                                        ...
                                        throw new Error('error: token format is not capitalized, or matches some reserved word, or does not start with \'ROLE_\'. token \''+tokens[index].symbol+'\', line '+tokens[index].line+', position: '+tokens[index].position+'. [from: '+from+']');
                                    }
                                }
                                code_body = code_body + '      ' + tokens[index].symbol + '\n'; // o ']' do 'read ['
                                index++;
                                ...
                                throw new Error('error: unknow token \''+tokens[index].symbol+'\', line '+tokens[index].line+', position: '+tokens[index].position+'. [from: '+from+']');
                            }
                        }
                        code_body = code_body + '    ' + tokens[index].symbol + '\n'; // o ')' do 'accessControl ('
                        index++;
                    ...
                    if ((!ycl_transpiler.ycl_reserved_word_contains(actual_attribute.name) 
                        ...
                        if (actual_attribute.command == 'd') {
                            ...
                            if (tokens[index].symbol == '(') {
                                ...
                                if (index == tokens.length) {
                                    throw new Error ('error: neither a next attribute was found, nor the end of the code after the attribute declared to be delete.');
                                }
                            } else {
                                    ...
                                    /* if (tokens[index].symbol == 'unique' || tokens[index].symbol == '!unique'
                                        ...
                                        if (symbol[symbol.length - 1] == 'String') {
                                            if (!isNaN(tokens[index + 1].symbol)) {
                                                ...
                                                throw new Error('error: unknow token. token \''+tokens[index + 1].symbol+'\', line '+tokens[index + 1].line+', position: '+tokens[index + 1].position+'. [from: '+from+']');
                                            }
                                        } else {
                                        ...
                                        throw new Error('error: unknow token. token \''+tokens[index].symbol+'\', line '+tokens[index].line+', position: '+tokens[index].position+'. [from: '+from+']');
                                    }
                                }
                                code_body = code_body + '    ' + tokens[index].symbol + '\n';
                                index++;   
                        ...
                        throw new Error('error: token matches some reserved word or length is less than 2. token \''+tokens[index].symbol+'\', line '+tokens[index].line+', position: '+tokens[index].position+'. [from: '+from+']');
                    }
                }
                code_body = code_body + '  ' + tokens[index].symbol + '\n'; // o '}' do 'entity <name> {'
                index++;
    ...
    deploy: function(model, callback) {
        ...
        } else if (model['_conf']['enabled']['command'] == 'u') {
                    ...
                    if (model['entities'][key2]['command'] == 'c') {
                        ...
                        Object.keys(model['entities'][key2]['attributes']).forEach(function(key4) {
                            ...
                            if (ycl_transpiler.types.includes(type_.value)) {
                                ...
                                if (model['entities'][key2]['attributes'][key4]['_conf']['nullable']) {
                                    association_.nullable = model['entities'][key2]['attributes'][key4]['_conf']['nullable'].value
                                }
                                /* if (model['entities'][key2]['attributes'][key4]['_conf']['unique']) {
                            ...
                            if (control.value) {
                                ...
                                if (ycl_transpiler.types.includes(type_.value)) {
                                    ...
                                    if (model['entities'][key2]['attributes'][key4]['command'] == 'c') {
                                        ...
                                        ycl_transpiler.deleteAttribute(schema.name, entity.name, attribute_.name, callback);
                                        control.value = false;
                                        ...
                                        ycl_transpiler.updateAttribute(schema.name, entity.name, attribute_.name, aux_attribute, callback);
                                        control.value = false;
                                    } else if (model['entities'][key2]['attributes'][key4]['_conf']['nullable']
                                        && model['entities'][key2]['attributes'][key4]['_conf']['nullable']['command'] == 'u') {
                                        ycl_transpiler.updateAttribute(schema.name, entity.name, attribute_.name, {
                                    ...
                                    if (model['entities'][key2]['attributes'][key4]['command'] == 'c') {
                                        ...
                                        ycl_transpiler.createAssociation(schema.name, entity.name, association_, callback);
                                        control.value = false;
                                    } else if (model['entities'][key2]['attributes'][key4]['command'] == 'd') {
                                        ycl_transpiler.deleteAssociation(schema.name, entity.name, association_.name, callback);
                                        control.value = false;
                                    } else if (model['entities'][key2]['attributes'][key4]['_conf']['nullable']
                                        && model['entities'][key2]['attributes'][key4]['_conf']['nullable']['command'] == 'u') {
                                        ycl_transpiler.updateAssociation(schema.name, entity.name, association_.name, {
                                            nullable: model['entities'][key2]['attributes'][key4]['_conf']['nullable']['value']
                                        }, callback);
                                        control.value = false;
                                    } else if (model['entities'][key2]['attributes'][key4]['_conf']['unique']
