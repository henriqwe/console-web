...
export type schemaType = {
  name?: string
  command?: string
  _conf?: { enabled: string }[]
  entities?: actual_entityType[]
}

export type actual_entityType = {
  name?: string
  command?: string
  _conf?: {
    dbType?: string
  ...
  attributes?: actual_attributeType[]
}
export type actual_attributeType = {
  name?: string
  command?: string
  _conf?: {
    unique?: {
...
export const ycl_reserved_words = [
  'schema',
  'entity',
  'enabled',
  'extends',
...
export const ycl_transpiler = {
  ...
  tokenizer_yc_code: function (text: string, entity_names_: string[]) {
    let tokens_ = []
  ...
  is_alphanum_lowercase: function (str: string) {
    return str.match(/^[a-z0-9]{2,}$/)
  },
  is_role_name_ok: function (str: string) {
    return str.match(/^ROLE_[A-Z]{2,}$/)
  },
  check_schema_object_name: function (str: string) {
    return str.match(/^[a-z0-9]{2,}$/)
  },
  ycl_reserved_word_contains: function (token: string) {
  ...
  db_type: 'sql',
  parse: function (src: string) {
    let schema: schemaType = {}
    let actual_entity: actual_entityType = {}
    let actual_attribute: actual_attributeType = {}
    ...
    while (index < tokens.length) {
      if (from == 0 && tokens[index].symbol == 'schema') {
        ...
        while (tokens[index].symbol != ')') {
          if (
            tokens[index].symbol == 'nosql' ||
            tokens[index].symbol == 'sql'
          ) {
            ...
            tokens[index].symbol == 'persistence' &&
            tokens[index + 1].symbol == '('
          ) {
            code_body =
              code_body +
              ...
              if (
                ...
                while (tokens[index].symbol != ']') {
                  if (
                    ...
                    actual_entity._conf!.uniqueKey.values!.push(
                      tokens[index].symbol
                    )
                    code_body =
                ...
                while (flag) {
                  if (
                    ...
                    if (
                      actual_entity._conf!.uniqueKey.partitionKeys!.values!
                        .length == 0
                    ) {
                ...
                while (tokens[index].symbol != ']') {
                  if (
                    ...
                    actual_entity._conf!.indexKey.values.push(
                      tokens[index].symbol
                    )
                    code_body =
                      code_body + '        ' + tokens[index].symbol + '\n'
              ...
              if (
                ...
                while (tokens[index].symbol != ']') {
                  if (
                    ...
                    console.log(
                      "error: token format is not capitalized, or matches some reserved word, or does not start with 'ROLE_'. token '" +
                        tokens[index].symbol +
                        "', line " +
                        tokens[index].line +
                        ', position: ' +
        ...
        while (tokens[index].symbol != '}') {
          {
            let aux: string | string[] = tokens[index].symbol
                ...
                if (
                  ...
                  if (symbol[symbol.length - 1] == 'String') {
                    if (!isNaN(tokens[index + 1].symbol)) {
                      ...
                      ycl_transpiler.types.includes(tokens[index + 1].symbol) ||
  ...
  deploy: function (model: schemaType, callback: callbackType) {
    const schema = {
      name: model['name'],
    ...
    if (model['command'] == 'c') {
      ...
      Object.keys(model['entities']!).forEach(function (idx2, key2) {
        if (control.value) {
          ...
          if (model['entities']![key2]['command'] == 'c') {
            ...
            if (
              ...
              entity._conf.accessControl!.read =
                _conf!.accessControl.read.values
            }
            if (
              _conf!.accessControl &&
            ...
            if (
              _conf!.dbType &&
              (_conf!.dbType == 'nosql(columnar)' ||
                _conf!.dbType == 'nosql(graph)')
            ) {
            ...
            model['entities']![key2]['_conf']!['businessRule']!['command'] ==
              'u'
          ) {
            ycl_transpiler.updateEntity(
              schema.name!,
            ...
            model['entities']![key2]['_conf']!['accessControl']!['command'] ==
              'u'
          ) {
            ycl_transpiler.updateEntity(
              schema.name!,
  ...
  createEntity: function (
    ...
    endpoint.url = endpoint.url.replace('{schemaName}', schema)
